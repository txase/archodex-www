...
const getSite = (config: Config) => {
  const _default = {
    ...
    site: `https://${import.meta.env.PUBLIC_ARCHODEX_DOMAIN}`,
    base: '/',
    trailingSlash: false,
...
const getMetadata = (config: Config) => {
  ...
  const _default = {
    title: { default: siteConfig?.name || DEFAULT_SITE_NAME, template: '%s' },
    description: '',
...
const getI18N = (config: Config) => {
  const _default = { language: 'en', textDirection: 'ltr' };
